...
const COLUMNS = [
    ...
    {
        key: "arn", label: "ARN",
        render: (row) => <span className="fw-semibold text-success">{row.arn}</span>
    },
    { key: "processedDate", label: "Processed Date" },
];

const SuccessWithARNReport = () => {
    ...
    return (
        <ReportBase
            title="Success with ARN Report"
            reportKey="SuccessWithARN"
            columns={COLUMNS}
            data={data}
            searched={searched}
            ...
            onToDate={setToDate}
            note="Lists all successfully processed refund transactions where an ARN was generated."
        />
    );
};

export default SuccessWithARNReport;
